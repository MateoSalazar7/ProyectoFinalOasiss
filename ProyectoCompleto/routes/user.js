...
router.post('/forgotPassword', (req, res) => {
    const user = req.body;
    query = "select email,passwrod from user where email=?";
    connection.query(query, [user.email], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
                ...
                var mailOptions = {
                    ...
                    to: results[0].email,
                    subject: 'Password by Discoteca El Oasis',
                    html: '<p><b>Your Login details for Discoteca El Oasis</b><br><b>Email:</b>' + results[0].email + '<br><b>Password: </b>' + results[0].password + '<br><a href="http://localhost:4200/">Clisck here to login</a></p>'
                };
